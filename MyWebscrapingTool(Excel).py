...
#of keywords along with their frequency and density in the webpage.The result for each webpage
#is stored in a worksheet and the whole batch is stored in a workbook.
...
#write() writes a list L OF tuples (word,(freq,density)) into a worksheet(sheet) along with the URL frm which each word was taken      
def write(sheet,L,url): 
     sheet.write(0,0,"Word")                        #Creating column names in the worksheet
     sheet.write(0,1,"Frequency")
     sheet.write(0,2,"Density")
     sheet.write(0,3,"URL")
     ctr=1                                          #row number in the worksheet
     for item in L:                                 #iterating through the List          
        #sheet.write(ctr,0,url )
        sheet.write(ctr,0,item[0])                  #writing word into column1
        sheet.write(ctr,1,item[1][0])               #frequency into column2
        sheet.write(ctr,2,item[1][1])               #density into column3
        sheet.write(ctr,3,url)                      #Url int col4
        ctr=ctr+1                                   #incrementing row number for next keyword input
...
WbName=input("Enter name of Workbook:")                                             #User inputs the name of the workbook 
workbook = xlsxwriter.Workbook("C:\\Users\\kol-00530-facnode\\Desktop\\Webscraping\\"+WbName+".xlsx")             #Default directory for WorkBooks
...
for Url in UrlList:                                                             
    ...
    SortedDictionary = sorted(MyDictionary.items(), key=lambda t:t[1],reverse=True) #Sorts the dicitonary according to frequency in descending order into a list of tuples
    worksheet=workbook.add_worksheet(SheetName)                                     #Creates new worksheet for the Url with name of the worksheet as that of the content scraped off the title tag of the page
    write(worksheet,SortedDictionary,Url)                                           #writes the table into the worksheet  
    print("")
